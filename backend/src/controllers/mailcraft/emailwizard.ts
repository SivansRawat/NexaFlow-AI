...
// Render deployment trigger: Embedded ChromaDB Vector Search Sync
...
export const sendMailChatMessage = async (req: Request, res: Response) => {
  try {
    ...
    // Retrieve RAG copywriting framework / template context if available
    let ragContext = '';
    try {
      ...
      console.log('MailCraft RAG retrieval skipped:', ragErr);
    }

    const systemPrompt = `You are an expert email writer and conversion copywriter. Write professional, persuasive, and clear emails.${
      ragContext ? `\n\n[RAG COPYWRITING FRAMEWORK & GUIDELINES]:\n${ragContext}` : ''
    }`;
    const userPrompt = `Write an email for the following request:\n\n${message}\n\nReturn only the completed email body.`;
