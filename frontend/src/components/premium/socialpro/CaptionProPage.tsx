...
const SAMPLE_TEMPLATES = [
  {
    ...
    content: 'Create an engaging Instagram caption for launching our new eco-friendly water bottle',
    platform: 'Instagram',
    industry: 'E-commerce',
    tone: 'Exciting'
  },
  {
    ...
    content: 'Write a LinkedIn post celebrating our team\'s successful project completion',
    platform: 'LinkedIn',
    industry: 'Technology',
    tone: 'Professional'
  },
  {
    ...
    content: 'Create a fun Instagram caption for a behind-the-scenes office photo',
    platform: 'Instagram',
    industry: 'Corporate',
    tone: 'Casual'
  },
  {
    ...
    content: 'Write a LinkedIn post sharing insights about the future of AI in healthcare',
    platform: 'LinkedIn',
    industry: 'Healthcare',
    tone: 'Educational'
  }
...
const CaptionProPage = () => {
  ...
  const loadLatestChatFromServer = async () => {
    try {
      const res = await fetch(`${API_BASE}/socialpro/captionpro/chats`, { headers: getAuthHeaders() });
      if (!res.ok) return;
      ...
      if (latest) {
        ...
        localStorage.setItem('captionPro_chatId', String(latest.id));
      }
    } catch (e) {
      console.error('Failed to load latest caption chat:', e);
    }
  };
  ...
  useEffect(() => {
    const init = async () => {
      const lastChatId = localStorage.getItem('captionPro_chatId');
      if (lastChatId) {
        try {
          const res = await fetch(`${API_BASE}/socialpro/captionpro/chat/${lastChatId}`, { headers: getAuthHeaders() });
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/socialpro/captionpro/chat/send`, {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        localStorage.setItem('captionPro_chatId', data.chat_id);
    ...
    <div className="flex flex-col w-11/12 max-w-5xl mx-auto bg-[#181c2a] rounded-2xl shadow-2xl border border-blue-900 overflow-hidden min-h-[85vh] mt-6">
      ...
      <div className="flex items-center gap-3 px-8 py-6 border-b border-blue-900 bg-gradient-to-r from-[#23263a] to-[#1e2235]">
        ...
        <div className="flex-1">
          <h2 className="font-bold text-2xl text-white">CaptionPro</h2>
          <p className="text-sm text-blue-300">Generate engaging social media captions with AI</p>
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-6 items-center">
          ...
          <div className="flex items-center gap-3">
            ...
            <select
              ...
              className="px-4 py-2.5 rounded-lg border border-blue-900 bg-[#181c2a] text-blue-100 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition-all duration-200"
              aria-label="Select caption tone"
            >
      ...
      <div className="flex-1 overflow-y-auto px-8 py-8 space-y-6 bg-[#181c2a]">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-blue-300">
            <p className="text-lg font-medium">Start by selecting a template or describing your caption needs below.</p>
            <p className="text-sm text-blue-400 mt-2">Choose your platform, industry, and tone for better results!</p>
          </div>
        ) : (
        ...
        {isLoading && (
          <div className="flex justify-start items-end">
            <div className="max-w-[80%] px-5 py-4 rounded-2xl shadow-lg text-base bg-[#23263a] text-blue-100 rounded-bl-md border border-blue-800 flex items-center">
              <span className="mr-2">Crafting your caption</span>
