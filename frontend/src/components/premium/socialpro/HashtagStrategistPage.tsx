...
const SAMPLE_TEMPLATES = [
  {
    ...
    content: 'Generate hashtags for promoting our new fitness app on Instagram',
    platform: 'Instagram',
    industry: 'Fitness & Health',
    contentContext: 'Mobile app launch, fitness tracking, wellness'
  },
  {
    title: 'Professional Networking',
    content: 'Create hashtags for a LinkedIn post about AI trends in business',
    platform: 'LinkedIn',
    industry: 'Technology',
    contentContext: 'Artificial intelligence, business innovation, future of work'
  },
  {
    title: 'Event Promotion',
    content: 'Generate hashtags for promoting our tech conference on Twitter',
    platform: 'Twitter',
    industry: 'Technology',
    contentContext: 'Tech conference, networking, innovation, speakers'
  },
  {
    title: 'Brand Awareness',
    content: 'Create hashtags for our fashion brand\'s summer collection',
    platform: 'Instagram',
    industry: 'Fashion',
    contentContext: 'Summer fashion, new collection, style trends'
  }
];

const HashtagStrategistPage = () => {
  const { user } = useAuth();
  ...
  const loadLatestChatFromServer = async () => {
    try {
      const res = await fetch(`${API_BASE}/socialpro/hashtagstrategist/chats`, { headers: getAuthHeaders() });
      if (!res.ok) return;
      ...
      if (latest) {
        ...
        localStorage.setItem('hashtagStrategist_chatId', String(latest.id));
      }
    } catch (e) {
      console.error('Failed to load latest hashtag chat:', e);
    }
  };
  ...
  useEffect(() => {
    const init = async () => {
      const lastChatId = localStorage.getItem('hashtagStrategist_chatId');
      if (lastChatId) {
        try {
          const res = await fetch(`${API_BASE}/socialpro/hashtagstrategist/chat/${lastChatId}`, { headers: getAuthHeaders() });
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/socialpro/hashtagstrategist/chat/send`, {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        localStorage.setItem('hashtagStrategist_chatId', data.chat_id);
        // Replace the temporary user message with the actual chat data
        setMessages(data.messages);
      } else {
  ...
  const handleTemplate = (template: any) => {
    ...
    setContent(template.contentContext);
  };
    ...
    <div className="flex flex-col w-11/12 max-w-5xl mx-auto bg-[#181c2a] rounded-2xl shadow-2xl border border-blue-900 overflow-hidden min-h-[85vh] mt-6">
      ...
      <div className="flex items-center gap-3 px-8 py-6 border-b border-blue-900 bg-gradient-to-r from-[#23263a] to-[#1e2235]">
        ...
        <div className="flex-1">
          <h2 className="font-bold text-2xl text-white">Hashtag Strategist</h2>
          <p className="text-sm text-blue-300">Generate trending and relevant hashtags with AI</p>
        </div>
      </div>
      ...
      <div className="px-8 py-5 bg-[#23263a] border-b border-blue-900">
        <div className="flex flex-wrap gap-6 items-center">
          ...
          <div className="flex items-center gap-3">
            <label htmlFor="industry-input" className="text-blue-200 text-sm font-medium">Industry:</label>
            <input
              id="industry-input"
              type="text"
          ...
          <div className="flex items-center gap-3">
            <label htmlFor="content-input" className="text-blue-200 text-sm font-medium">Content Context:</label>
            <input
              id="content-input"
              type="text"
              placeholder="e.g., Product launch, event, post topic"
              value={content}
      ...
      <div className="flex-1 overflow-y-auto px-8 py-8 space-y-6 bg-[#181c2a]">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-blue-300">
            <p className="text-lg font-medium">Start by selecting a template or describing your hashtag needs below.</p>
            <p className="text-sm text-blue-400 mt-2">Choose your platform, industry, and content context for better results!</p>
          </div>
        ) : (
        ...
        {isLoading && (
          <div className="flex justify-start items-end">
            <div className="max-w-[80%] px-5 py-4 rounded-2xl shadow-lg text-base bg-[#23263a] text-blue-100 rounded-bl-md border border-blue-800 flex items-center">
              <span className="mr-2">Analyzing hashtag trends</span>
      ...
      <div className="px-8 py-6 bg-[#23263a] flex items-center gap-3">
        <textarea
          ...
          placeholder="Describe the hashtags you need or your content topic..."
          value={input}
          onChange={e => {
...
export default HashtagStrategistPage;
