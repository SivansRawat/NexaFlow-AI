...
const SAMPLE_TEMPLATES = [
  {
    ...
    content: 'Write a job application email for a software engineer position at Google.'
  },
  {
    title: 'Leave Request',
    content: 'Draft an email requesting 2 days of leave for a family emergency.'
  },
  {
    title: 'Sales Follow-up',
    content: 'Write a follow-up email to a client after a sales meeting.'
  },
  {
    title: 'Meeting Scheduling',
    content: 'Draft an email to schedule a meeting with a project team next week.'
  }
];

const EmailWizardPage = () => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const lastChatId = localStorage.getItem('emailWizard_chatId');
    if (lastChatId) {
      fetch(`${API_BASE}/mailcraft/emailwizard/chat/${lastChatId}`, {
        headers: getAuthHeaders() // Add auth headers here too
      })
        .then(res => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        localStorage.setItem('emailWizard_chatId', data.chat_id);
        setMessages(data.messages);
        setInput('');
    ...
    <div className="flex flex-col w-11/12 max-w-4xl mx-auto bg-[#181c2a] rounded-2xl shadow-2xl border border-blue-900 overflow-hidden min-h-[80vh] mt-8">
      ...
      <div className="flex items-center gap-3 px-8 py-6 border-b border-blue-900 bg-[#23263a]">
        ...
        <div className="flex-1">
          <h2 className="font-bold text-2xl text-white">Email Wizard</h2>
          <p className="text-xs text-blue-300">Generate professional emails with AI</p>
        </div>
      </div>
      {/* Sample Templates */}
      <div className="px-8 py-4 bg-[#23263a] border-b border-blue-900">
        <div className="mb-2 text-blue-200 font-semibold">Popular Templates:</div>
      ...
      <div className="flex-1 overflow-y-auto px-8 py-8 space-y-6 bg-[#181c2a]">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-blue-300">
            ...
            <p className="text-lg">Start by selecting a template or typing your email request below.</p>
          </div>
        ) : (
...
export default EmailWizardPage;